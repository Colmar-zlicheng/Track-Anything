...
def parse_augment():
    ...
    parser.add_argument('-m', '--mask_save', type=bool, default=True)
...
def select_template(image_selection_slider, video_state, interactive_state):
    ...
    operation_log = [("", ""),
                     ("Select frame {}. Try click image and add mask for tracking.".format(image_selection_slider),
                      "Normal")]
...
def add_multi_mask(video_state, interactive_state, mask_dropdown):
    try:
        ...
        interactive_state["multi_mask"]["mask_names"].append("mask_{:03d}".format(
...
def vos_tracking_video(video_state, interactive_state, mask_dropdown):
    ...
    if interactive_state["multi_mask"]["masks"]:
        ...
        mask_dropdown.sort()
        template_mask = interactive_state["multi_mask"]["masks"][int(mask_dropdown[0].split("_")[1]) - 1] * (int(
            mask_dropdown[0].split("_")[1]))
    ...
    video_output = generate_video_from_frames(video_state["painted_images"],
                                              output_path=os.path.join(
                                                  interactive_state['save_path'],
                                                  f"{video_state['video_name'].split('.')[0]}_segment.mp4"),
    ...
    sub_mask_dir = os.path.join(object_mask_path, video_state['video_name'].split('.')[0])
    os.mkdir(sub_mask_dir)
    if interactive_state["mask_save"]:
        i = int(video_state['video_name'].split('_')[0])
        print("save mask")
        for mask in etqdm(video_state["masks"]):
            Image.fromarray(mask * 255).save(os.path.join(sub_mask_dir, '{:04d}.png'.format(i)))
            i += 1
...
if __name__ == '__main__':
    ...
    with gr.Blocks() as iface:
            ...
            with gr.Column():
                with gr.Row(scale=0.4):
                    ...
                    with gr.Column():
                        ...
                        resize_info = gr.Textbox(
                            value=
                            "If you want to use the inpaint function, it is best to git clone the repo and use a machine with more VRAM locally. \
                        ...
                        resize_ratio_slider = gr.Slider(minimum=0.02,
                                                        maximum=1,
                                                        step=0.02,
                                                        value=1,
                                                        label="Resize ratio",
                ...
                with gr.Row():
                    ...
                    with gr.Column():
                        # extract frames
                        with gr.Column():
                            extract_frames_button = gr.Button(value="Get video info",
                                                              interactive=True,
                                                              variant="primary")
                        ...
                        with gr.Row():
                            with gr.Row():
                                point_prompt = gr.Radio(choices=["Positive", "Negative"],
                                                        value="Positive",
                                                        label="Point Prompt",
                                                        interactive=True,
                                                        visible=False)
                                ...
                                clear_button_click = gr.Button(value="Clear Clicks", interactive=True,
                                                               visible=False).style(height=160)
                        ...
                        image_selection_slider = gr.Slider(minimum=1,
                                                           maximum=100,
                                                           step=1,
                                                           value=1,
                                                           label="Image Selection",
                                                           visible=False)
                        track_pause_number_slider = gr.Slider(minimum=1,
                                                              maximum=100,
                                                              step=1,
                                                              value=1,
                    ...
                    with gr.Column():
                        run_status = gr.HighlightedText(value=[("Text", "Error"), ("to be", "Label 2"),
                                                               ("highlighted", "Label 3")],
                                                        visible=False)
                        mask_dropdown = gr.Dropdown(multiselect=True,
                                                    value=[],
        ...
        extract_frames_button.click(fn=get_frames_from_video,
        ...
        # set example
        gr.Markdown("##  Your video (Choose the video below)")
        gr.Examples(
            examples=[
                os.path.join(args.input_dir, test_sample)
                for test_sample in sorted(os.listdir(args.input_dir), key=lambda x: int(x.split('_')[0]))
            ],
            fn=run_example,
            inputs=[video_input],
            outputs=[video_input],
